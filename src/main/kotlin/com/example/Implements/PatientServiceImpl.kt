...
class PatientServiceImpl(private val db: CoroutineDatabase, private val NotificationService: Notification) :
    ...
    override suspend fun getAvaiableSlots(doctorid: String, date: Int, month: Int, year: Int): List<Int> {
        try {
            var doc = doctorCollection.findOne(Doctor::id eq ObjectId(doctorid))
            if (doc != null) {
                val start = doc.working_hour_start ?: 0
                val end = doc.working_hour_end ?: 23
                val totalslots: HashSet<Int> = HashSet()
                totalslots.addAll(start..end)
                println("size of slots is ${totalslots.size}")
                var filter =
                    Filters.and(AppointMents::date eq date, AppointMents::month eq month, AppointMents::year eq year)
                var appointments = AppointmentCollection.find(filter).toList()
                var bookedappointment: HashSet<Int> = HashSet()
                appointments.forEach {
                    bookedappointment.add(it.time)
                }
                var res: Set<Int> = totalslots - bookedappointment
                println(bookedappointment.toArray())
                println(res.toMutableList())
                println(totalslots.toMutableList())
                return res.toList()

            }
            else {
                throw Exception("Doctor not Available with such id")
            }
        }
     catch (e: Exception){
    ...
    try {
        ...
        if (doctor != null && patient != null) {
            val newappointment = AppointMents(
                ...
                url = url,
                doctorname = doctor.fullname ?: "DocMate Doctor",
                patientname = patient.name ?: "DocMate Patient"
                )
